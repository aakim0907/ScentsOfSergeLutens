...
for (const key of Object.keys(updatedPerfumeList)) {
  ...
  val['notes'].forEach( n => {
    if (!notes.includes(n)) {
      notes.push(n);
      perfumeObj['nodes'].push({'id': notes.indexOf(n), 'name': n});
    }
  });
  ...
  for (let i = 0; i < val['notes'].length - 1; i++) {
    for (let j = i + 1; j < val['notes'].length; j++) {
      ...
      const firstId = notes.indexOf(firstNote),
            secondId = notes.indexOf(secondNote),
      ...
      if (typeof existingLink === 'undefined') {
        links.push(link);
        perfumeObj['links'].push({'source': link[0], 'target': link[1], 'value': 1});
      } else {
        const temp = perfumeObj['links'].find( ele => {
          return (ele.source === firstId && ele.target === secondId)
            || (ele.source === secondId && ele.target === firstId);
        });
        temp.value++;
      }
    }
